sap.ui.define([
	...
	function fnExtractErrorContentFromResponse(sResponse) {
		...
		try {
			...
			try {
				...
				if (xmlDoc && xmlDoc.getElementsByTagName("message")[0]) {
					...
					oError.sMessage = xmlDoc.childNodes[0].childNodes[1].textContent;
					oError.sDetails = null;
					oError.aInnerErrors = [xmlDoc.childNodes[0].childNodes[1].textContent];
				}
			} catch (f) {
	...
	function fnParseError(oEvent) {
		...
		if (responseContent === "" && oResponse.message) {
			responseContent = oResponse.message;
		}
	...
	return UI5Object.extend("com.sap.zsdxcrdinsu.controller.ErrorHandler", {
		/*
		...
		constructor: function (oComponent, oMessageProcessor, oMessageManager) {
			...
			//in case initial loading of metadata failed, retrigger metadata loading so that we show
			//a proper error message Popup
			//-> relevant since "preload"-flag for the ODATA-model was set in manifest.json
		 ...
		 * @private
		 */
		_showServiceError: function (sErrorText, sErrorDetails, sTitleText, fnCallback) {
		 ...
		 */
		displayErrorPopup: function (fnCallback) {
			if (this._aErrors.length === 0) {
				return false;
			}
			this._showServiceError(this._aErrors.join("\n"), null, null, fnCallback);
			this._aErrors = [];
